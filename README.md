...
1. PC 通道就绪后，按钮显示 **「开始准备」**（唯一需要手部点击的步骤）
2. 进入 XR 后 **3 秒倒计时**（画面中央大数字 + 文案：请摆成与机械臂相近的姿势，双手不要再点击）
...
1. 在 Quest Browser 中打开 `https://<PC-IP>:8000/`，首次访问时接受自签名证书；PC 通道就绪后点击 **「开始准备」**（唯一点击）。进入 XR 后 **3 秒倒计时**（摆姿，勿再点 UI），结束后 **自动开始传输**。
